...
os.system('cls')
os.system("title Doxxer ^| Vers 1.0 ^| Made By Synk ^|")
...
gui = """                                                                                                                                                                                                                                                                                                                                                                                                                                                                     
                    ...
                    ╚════╦═══════════════════════════════════════════╦══════╝ ╚═╦═══════════╦═╝
                         ║                                           ║          ║           ║     
                       ╔═╩═══════════════════════════════════════════╩═╗ ╔══════╩═══════════╩══════╗
                       ║      [ https://discord.gg/RXvb6U2c7C ]        ║ ║  LETS DESTROY THE WORLD ║
                       ╚═══════════════════════════════════════════════╝ ╚═════════════════════════╝
"""
faded_text = fade.brazil(gui)
print(faded_text)

username = input(
    Fore.WHITE + '                       [!] Enter name: ')
os.system(
    f"title Doxxed {username} ^| Estimated time: 1 Minute ^| LETS DESTROY THE WORLD")
...
WEBSITES = [
    ...
    medium, deviantart, aboutme, imgur, flipboard, slideshare, fotolog, spotify,
...
def search():
        ...
        if r.status_code == 200:
            if match == True:
                print(Fore.LIGHTYELLOW_EX +
                      '                                                [+] Results')
                match = False
            YELLOW(Fore.LIGHTBLACK_EX +
                   f'\n                             [ + ] - {url} - {r.status_code} - OK - [ + ]')
            if username in r.text:
                print(Fore.LIGHTCYAN_EX +
                      f'                                    {locijl}', Fore.LIGHTGREEN_EX + f'{username} --> L I V E <--')
            else:
                print(Fore.LIGHTCYAN_EX +
                      f'                                    {locijl}', Fore.LIGHTRED_EX + f'{username} --> D E A D <--')
        count += 1
    ...
    print(faded_text)
    print(Fore.LIGHTCYAN_EX +
          f' -> END! - have been detected {count} accounts. | In {total} different pages.')

    print(bcolors.Redd + 'test')
